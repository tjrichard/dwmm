...
const getWorksDataSourceId = async () => {
  ...
  cachedWorksDataSourceId = await resolveDataSourceId({
    databaseId: process.env.NOTION_DATABASE_ID || process.env.NOTION_WORKS_DATABASE_ID,
    dataSourceId: process.env.NOTION_DATABASE_ID ? undefined : process.env.NOTION_WORKS_DATA_SOURCE_ID,
    name: 'DWMM',
    envHint: 'Set NOTION_DATABASE_ID, NOTION_WORKS_DATABASE_ID, or NOTION_WORKS_DATA_SOURCE_ID in .env.local.',
  });
...
let cachedFavoritesDataSourceId = null;
const getFavoritesDataSourceId = async () => {
  if (cachedFavoritesDataSourceId) return cachedFavoritesDataSourceId;

  const directDataSourceId =
    process.env.NOTION_FAVORITES_DATA_SOURCE_ID || process.env.NOTION_FAVORIETS_DATA_SOURCE_ID;
  const directDatabaseId =
    process.env.NOTION_FAVORITES_DATABASE_ID || process.env.NOTION_FAVORIETS_DATABASE_ID;

  const attempts = [
    () =>
      resolveDataSourceId({
        dataSourceId: directDataSourceId,
        databaseId: directDatabaseId,
        envHint:
          'Set NOTION_FAVORITES_DATA_SOURCE_ID, NOTION_FAVORIETS_DATA_SOURCE_ID, NOTION_FAVORITES_DATABASE_ID, or NOTION_FAVORIETS_DATABASE_ID in .env.local.',
      }),
    () =>
      resolveDataSourceId({
        name: 'favoriets',
        envHint:
          'Set NOTION_FAVORITES_DATA_SOURCE_ID or share the Notion favorites datasource with the integration.',
      }),
    () =>
      resolveDataSourceId({
        name: 'favorites',
  ...
  for (const attempt of attempts) {
    try {
      cachedFavoritesDataSourceId = await attempt();
      return cachedFavoritesDataSourceId;
    } catch (error) {
      lastError = error;
    }
  }

  throw lastError || new Error('Favorites data source could not be resolved.');
};
...
export const getFavoriteEntries = async () => {
  const dataSourceId = await getFavoritesDataSourceId();
  const response = await withRequestCache(`favorites:list:${dataSourceId}`, 1000 * 60 * 5, async () =>
  ...
  return (response.results || [])
    .map((page) => {
      const properties = page.properties || {};
      const titleProperty =
        findProperty(properties, ['title', 'name', 'item', 'favorite', 'favourite', '이름', '제목']) ||
        findTitleProperty(properties);
      const summaryProperty = findProperty(properties, [
        'summary',
        'description',
        'note',
        'memo',
      ...
      const categoryProperty = findProperty(properties, [
        'category',
        'type',
        'group',
        'topic',
      ...
      const dateProperty = findProperty(properties, [
        'date',
        'day',
        'event_date',
        'target_date',
      ...
      const urlProperty = findProperty(properties, [
        'url',
        'link',
        'website',
        'site',
      ...
      return {
        id: page.id,
        title: getPropertyText(titleProperty) || 'Untitled favorite',
        summary: getPropertyText(summaryProperty),
        category: getPropertyText(categoryProperty) || 'Unsorted',
        type: getPropertyText(categoryProperty) || 'Unsorted',
        date: getDateRangeFromProperty(dateProperty).start || '',
        href: getPropertyUrl(urlProperty) || page.url || null,
        image: getPropertyUrl(imageProperty) || getNotionFileUrl(page.cover) || null,
    ...
    .sort((a, b) => {
      const aDate = getSortDateValue(a.updatedAt);
      const bDate = getSortDateValue(b.updatedAt);
